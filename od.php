...
if ($result_subquery->num_rows > 0) {
    ...
    // Date check
    $sql_check = "SELECT * FROM applyod WHERE uname = '$uname' AND fd = '$fd' AND noo = '$noo'";
    $result_check = $conn->query($sql_check);
    if ($result_check->num_rows > 0) {
        $_SESSION['error_message'] = "You have already applied OD in the entered dates!!!";
        header('Location: od.html');    
        exit();
    }

    // Insert into applyod table
    $sql = "INSERT INTO applyod (name, uname, regno, branch, year, sec, noo, nod, fd, td, dd, proof, es, smark, mmark, cmark, hmark, mail, mentormail, coordmail)
    VALUES ('$name', '$uname', '$regno', '$branch', '$year', '$sec', '$noo', '$nod', '$fd', STR_TO_DATE('$td', '%d-%m-%Y'), '$dd', '$proof', '$es', true, 'false', 'false', 'false', '$email', '$mentormail', '$coordmail')";


    if ($conn->query($sql) === TRUE) {
        $_SESSION['success_message'] = "Your OD is applied successfully. Please wait for the approval.";
        header('Location: od.html');
        exit();
    } else {
